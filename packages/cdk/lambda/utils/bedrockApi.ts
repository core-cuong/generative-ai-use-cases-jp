...
const createBodyText = (
  ...
  const modelConfig = BEDROCK_MODELS[model as keyof typeof BEDROCK_MODELS];
...
const createBodyImage = (
  ...
  const modelConfig =
    BEDROCK_IMAGE_GEN_MODELS[model as keyof typeof BEDROCK_IMAGE_GEN_MODELS];
...
const bedrockApi: ApiInterface = {
  ...
  generateImage: async (model, params) => {
    ...
    return extractOutputImage(model.modelName, body);
  },
};
