...
const Setting = () => {
  ...
  const modelNames = MODELS.textModels.map((m) => m.modelName).join(', ');
  const imageGenModelNames = MODELS.imageGenModels
    .map((m) => m.modelName)
    .join(', ');
  ...
  return (
    <div>
      ...
      <div className="flex w-full flex-col items-center text-sm">
        <SettingItem name="LLM モデル名" value={modelNames} />
        <SettingItem name="画像生成 モデル名" value={imageGenModelNames} />
        <SettingItem
