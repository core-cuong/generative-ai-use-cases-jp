...
`cdk.json` の `modelRegion`, `modelNames`, `imageGenerationModelNames` でモデルとモデルのリージョンを指定します。`modelNames` と `imageGenerationModelNames` は指定したリージョンで利用できるモデルの中から利用したいモデルをカンマ区切りで指定してください。
